...
https://doylead.me/?p=19

The goal of this script is to generate data which follow a basic linear relationship and to show
two different methods of calculating the parameters for the best linear fit.  We show that an
approach based on the dot product in an abstract vector space matches trusted results from
scikit-learn
'''
...
# We extract scalar values from the return format scikit-learn provides
skl_m = reg.coef_[0][0]
skl_b = reg.intercept_[0]

# Fit a model using our techniques (specifically using Equation (6) and abstract approaches).  We
#  need the transpose to keep inner dimensions consistent
x_mean = np.average(x)
y_mean = np.average(y)
x_T = np.transpose(x)

# The output of both dot products is a 1x1 matrix. We use array[0][0] to extract the first/only
